...
def main(imgno):
    ...
    nodes_from_root.append(Node(start[0], start[1]))  # Start in the corner (upper left)
    nodes_from_goal.append(Node(end[0], end[1]))  # Start in the corner (lower right)
    ...
    goal_root = start_goal
    goal_goal = start_root
    q_nearest = None
    ...
    pygame.display.update()
    pygame.image.save(screen, "./images/0423sample10/birrt-star/random/image" + str(imgno) + ".jpg")
    print(nodes_expanded, elapsed, path_len)
    #with open(r'./result-0423sample10/birrt-star/p0.2/birrt-star-0423sample10-p0.2.csv', 'a', newline='') as file:
    #    writer = csv.writer(file)
    #    writer.writerow([nodes_expanded, elapsed, path_len])
    pygame.quit()
